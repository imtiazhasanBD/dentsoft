...
const calculateIndividualToothCost = (procedures) => {
  ...
  return procedures.reduce((sum, procName) => {
    return sum + (procedureCosts[procName] || procedureCosts["Default Procedure"] || 0);
  }, 0);
};
...
export default function TreatmentDialog({ open, onOpenChange, toothNumbers, existingTreatment, onSaveTreatment }) {
      ...
      <DialogContent className="sm:max-w-2xl md:max-w-3xl lg:max-w-4xl bg-card h-[85vh] flex flex-col">
        <DialogHeader>
          <DialogTitle className="font-headline">
            {existingTreatment ? 'Edit Treatment' : 'Add Treatment'} for {toothSpecificDetails.length > 1 ? 'Teeth' : 'Tooth'} {toothSpecificDetails.map(td => td.toothNumber).join(', ')}
          </DialogTitle>
          <DialogDescription>
            Log details for the selected {toothSpecificDetails.length > 1 ? 'teeth' : 'tooth'}.
          </DialogDescription>
        </DialogHeader>
        
        <ScrollArea className="flex-grow pr-6 -mr-6 overflow-y-auto">
          <div className="space-y-4 py-4">
            <Accordion type="multiple" defaultValue={toothSpecificDetails.map((_, idx) => `tooth-${idx}`)} className="w-full">
              {toothSpecificDetails.map((detail, index) => (
                <AccordionItem value={`tooth-${index}`} key={detail.toothNumber || index}>
                  <AccordionTrigger className="font-semibold">Treatment for Tooth {detail.toothNumber}</AccordionTrigger>
                  <AccordionContent>
                    <div className="space-y-4 p-2 border rounded-md">
                      <MultiSelectInput
                        ...
                        onChange={(items) => handleToothDetailChange(index, 'chiefComplaint', items)}
                      ...
                      <MultiSelectInput
                        ...
                        onChange={(items) => handleToothDetailChange(index, 'clinicalFindings', items)}
                      ...
                      />
                       <MultiSelectInput
                        ...
                        onChange={(items) => handleToothDetailChange(index, 'diagnosis', items)}
                      ...
                      />
                       <MultiSelectInput
                        ...
                        onChange={(items) => handleToothDetailChange(index, 'procedure', items)}
                        suggestions={importantProcedureOptions}
                        allAvailableSuggestions={allProcedureOptions}
                      />
                      <div className="p-2 bg-muted rounded-md">
                        <Label className="font-semibold">Cost for Tooth {detail.toothNumber}:</Label>
                        <p className="text-lg text-primary">${(detail.cost || 0).toFixed(2)}</p>
                      </div>
                      <div>
                        <Label htmlFor={`notes-tooth-${index}`}>Notes (Tooth Specific)</Label>
                        <Textarea id={`notes-tooth-${index}`} value={detail.notes} onChange={(e) => handleToothDetailChange(index, 'notes', e.target.value)} />
                      </div>
                    </div>
                  </AccordionContent>
                </AccordionItem>
            ...
            <div className="mt-6 p-4 border rounded-md space-y-4">
              <h3 className="text-lg font-semibold">Overall Treatment Summary</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="overallNotes">Overall Notes</Label>
                  <Textarea id="overallNotes" value={overallNotes} onChange={(e) => setOverallNotes(e.target.value)} placeholder="General notes for this treatment session" />
                </div>
                <div>
                  <Label htmlFor="overallStatus">Overall Status</Label>
                  <RadioGroup value={overallStatus} onValueChange={setOverallStatus} className="flex space-x-4 pt-2">
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="in-progress" id="r-overall-in-progress" />
                      <Label htmlFor="r-overall-in-progress">In Progress</Label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="completed" id="r-overall-completed" />
               ...
               <div className="p-2 bg-muted rounded-md">
                <Label className="font-semibold">Total Estimated Cost for Treatment:</Label>
                <p className="text-xl font-bold text-primary">
                  ${toothSpecificDetails.reduce((sum, detail) => sum + (detail.cost || 0), 0).toFixed(2)}
                </p>
              </div>
            </div>
          </div>
        ...
        <DialogFooter className="sm:justify-between mt-auto pt-4 border-t">
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={handleSubmit} disabled={!toothSpecificDetails.length}>
              Save Treatment
            </Button>
          </div>
        </DialogFooter>
